...
FanucM20iA::FanucM20iA(const double xMin, const double yMin, const double zMin,
	...
	: _portS(59002),
	  _portR(59003),
	  _tmpAddrString("192.168.0.21"),
	  _wsaData(),
	  _workInJoint(false),
	  _maxValueOf(xMax, yMax, zMax),
	  _minValueOf(xMin, yMin, zMin),
	  _homeposeWorld({{985., 0., 940., 180., 0., 0.}}),
	  _homeposeJoint({{0., 0., 0., 0., -90., 0.}})
{
	initSockets();
}
...
FanucM20iA::ExtremeValuesMax::ExtremeValuesMax(const double& x, const double& y, const double& z): _x(x),
                                                                                                   _y(y),
                                                                                                   _z(z)
{
}
...
FanucM20iA::ExtremeValuesMin::ExtremeValuesMin(const double& x, const double& y, const double& z): _x(x),
                                                                                                   _y(y),
                                                                                                   _z(z)
{
}
...
FanucM20iA::~FanucM20iA()
{
	if (_workInJoint)
	{
		goToCoordinates(_homeposeJoint.at(0), _homeposeJoint.at(1), _homeposeJoint.at(2),
		                _homeposeJoint.at(3), _homeposeJoint.at(4), _homeposeJoint.at(5));
		getJointAngles();
	}
	else
	{
		goToCoordinates(_homeposeWorld.at(0), _homeposeWorld.at(1), _homeposeWorld.at(2),
